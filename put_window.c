...
/*   Updated: 2023/05/29 01:25:36 by ssaadaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

void handle_image(t_list *data, char **map, void *image,int i, int j ) 
{
	(void)map;
...
void moves(t_list *data , int x, int y)
{
	if (data->map_2d[data->x_player + x][data->y_player + y] == '1' || 
	 (data->map_2d[data->x_player + x][data->y_player + y] == 'E' && data->count_C != 0))
		return ;
